...
const partnerTypes = ['Corporate', 'Foundation', 'Government', 'Community', 'NGO', 'Academic'];
...
export default function NewPartnerModal({ onClose }: NewPartnerModalProps) {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({
    name: '', type: '', website: '', contactName: '',
    contactEmail: '', contactPhone: '',
    contribution: '', startDate: '', projects: '',
    notes: '', status: 'active'
  });
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ color: 'white', fontSize: '22px', fontWeight: 400, fontFamily: 'Georgia, serif', margin: 0 }}>
              {step === 1 ? 'Partner Information' : 'Contribution & Details'}
            </h2>
            ...
            <>
              <div>
                ...
                <input value={form.name} onChange={e => set('name', e.target.value)}
                  placeholder="e.g. Gates Foundation" style={inputStyle}
              ...
              <div>
                ...
                <input value={form.website} onChange={e => set('website', e.target.value)}
                  placeholder="https://example.org" style={inputStyle}
                  onFocus={e => e.target.style.borderColor = '#0F4C3A'}
                  onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input type="email" value={form.contactEmail} onChange={e => set('contactEmail', e.target.value)}
                    placeholder="jane@example.org" style={inputStyle}
                    onFocus={e => e.target.style.borderColor = '#0F4C3A'}
                    onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
                </div>
              </div>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input type="number" value={form.contribution} onChange={e => set('contribution', e.target.value)}
                    placeholder="50000" style={inputStyle}
                    onFocus={e => e.target.style.borderColor = '#0F4C3A'}
                    onBlur={e => e.target.style.borderColor = '#e5e5e5'} />
                </div>
                <div>
                  <label style={labelStyle}>Partnership Since *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={labelStyle}>Projects Involved</label>
                  <input type="number" value={form.projects} onChange={e => set('projects', e.target.value)}
              ...
              {form.name && (
                <div style={{
                  ...
                  <p style={{ fontSize: '12px', color: '#666' }}>
                    ...
                    {form.contribution ? ` · $${Number(form.contribution).toLocaleString()}/yr` : ''}
                  </p>
                </div>
              )}
            </>
        ...
        <div style={{
          ...
          <button onClick={() => step === 1 ? setStep(2) : handleCreate()} style={{
            ...
            {step === 1 ? <><span>Continue</span><ChevronRight size={15} /></> : <><span>Add Partner</span><Plus size={15} /></>}
          </button>
        </div>
      </div>
    </div>
